...
public class Table implements TableOrView, TableSchema, Closeable {
        ...
        if (hasPrimaryKey()) {
            ...
            switch (type) {
                case STRING:
                    ...
                    break;
                case INTEGER:
                    if (findFirstLong(primaryKeyColumnIndex, 0) != -1) {
                        throwDuplicatePrimaryKeyException(Long.valueOf(0));
                        break;
                    }
                    break;
                default:
        ...
        switch (type) {
            case STRING:
                if (primaryKeyValue instanceof String) {
                    if (findFirstString(primaryKeyColumnIndex, (String) primaryKeyValue) != -1) {
                        throwDuplicatePrimaryKeyException(primaryKeyValue);
                    }
                    rowIndex = nativeAddEmptyRow(this.nativePtr, 1);
            ...
            case INTEGER:
                try {
                    ...
                    if (findFirstLong(primaryKeyColumnIndex, pkValue) != -1) {
        ...
        for (long columnIndex2 = 0; columnIndex2 < ((long) columns); columnIndex2++) {
            ...
            switch (colTypes[(int) columnIndex2]) {
                ...
                case MIXED:
                    ...
                    break;
                case BINARY:
                    nativeInsertByteArray(this.nativePtr, columnIndex2, rowIndex, (byte[]) value2);
                    break;
                case TABLE:
    ...
    public long getPrimaryKey() {
        ...
        long rowIndex = pkTable.findFirstString(0, getName());
        if (rowIndex != -1) {
    ...
    public String toString() {
        return nativeToString(this.nativePtr, -1);
    }
