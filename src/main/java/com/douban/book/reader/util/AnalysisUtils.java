...
public class AnalysisUtils {
    ...
    public static class ShareEventBuilder {
        ...
        public void send() {
            Analysis.sendEventWithExtra(AnalysisUtils.EVENT_SHARE, this.mContentType, ((JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) RequestParam.json().append(BaseShareEditFragment.KEY_SHARE_TO, this.mShareTo)).append("title", this.mContentTitle)).append("contentId", this.mContentId)).appendIfNotEmpty("description", this.mContentDescription)).append("uri", this.mContentUri)).append("relatedWorksId", this.mRelatedWorksId)).appendIfNotEmpty(HitTypes.TRANSACTION, this.mWeixinTransaction)).getJsonObject());
    ...
    public static String buildShareUrl(String url, String shareType, String dcm, String dcs) {
        ...
        builder.appendQueryParameter("type", shareType);
        return builder.build().toString();
    }

    public static void sendPurchaseEvent(final Uri purchasedUri) {
        TaskController.run(new Runnable() {
            public void run() {
                if (purchasedUri == null) {
                    return;
                }
                if (ReaderUriUtils.isWorksUri(purchasedUri) || ReaderUriUtils.isPackageUri(purchasedUri)) {
                    try {
                        int identifyId;
                        boolean isWorksUri = ReaderUriUtils.isWorksUri(purchasedUri);
                        boolean isChapterUri = ReaderUriUtils.isPackageUri(purchasedUri);
                        UserInfo userInfo = UserManager.getInstance().getUserInfo();
                        int worksId = ReaderUriUtils.getWorksId(purchasedUri);
                        int chapterId = ReaderUriUtils.getPackageId(purchasedUri);
                        Works works = (Works) WorksManager.getInstance().get((Object) Integer.valueOf(worksId));
                        double price = (double) (((float) ReaderUriUtils.getPrice(purchasedUri)) / 100.0f);
                        if (isWorksUri) {
                            identifyId = worksId;
                        } else {
                            identifyId = chapterId;
                        }
                        String productName = isWorksUri ? works.title : String.format("%s_%s", new Object[]{works.title, Package.get(purchasedUri).getTitle()});
                        ...
                        easyTracker.send(MapBuilder.createTransaction(transactionId, "Android", Double.valueOf(price), Double.valueOf(0.0d), Double.valueOf(0.0d), "CNY").build());
                        ...
                        Analysis.sendEventWithExtra("purchase", category, ((JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) RequestParam.json().append("title", productName)).append("price", Double.valueOf(price))).append("works_id", Integer.valueOf(worksId))).appendIf(isChapterUri, "chapter_id", Integer.valueOf(chapterId))).toString());
