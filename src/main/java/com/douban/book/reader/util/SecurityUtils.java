...
public class SecurityUtils {
    ...
    private static SecretKey getKey(byte[] salt, String password) throws Exception {
        try {
            return new SecretKeySpec(SecretKeyFactory.getInstance(KEY_DERIVATION_ALGORITHM, "BC").generateSecret(new PBEKeySpec(password.toCharArray(), salt, 1000, 256)).getEncoded(), "AES");
        } catch (Throwable e) {
