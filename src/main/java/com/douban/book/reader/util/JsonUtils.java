...
public class JsonUtils {
    private static Gson sGson = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ssZZZZZ").setFieldNamingPolicy(FieldNamingPolicy.LOWER_CASE_WITH_UNDERSCORES).registerTypeAdapterFactory(new StoreWidgetGsonTypeFactory()).create();
    ...
    public static <T> T fromStream(InputStream stream, Class<T> type) {
        return fromJson(new JsonReader(new InputStreamReader(stream)), (Class) type);
    }

    public static <T> T fromFile(File file, Class<T> type) throws IOException {
        return fromStream(new FileInputStream(file), type);
    }
