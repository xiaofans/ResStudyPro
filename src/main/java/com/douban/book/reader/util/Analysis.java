...
public class Analysis {
    ...
    public static void sendPageViewEvent(final String pageName, final Bundle params, final int visibleTimes) {
        TaskController.run(new Runnable() {
            public void run() {
                String formattedPageName = Analysis.formatPageName(pageName);
                Tracker tracker = EasyTracker.getInstance(App.get());
                tracker.set("&cd", formattedPageName);
                ...
                if (params != null) {
                    reducedParams = new Bundle(params);
                    reducedParams.remove(GeneralFragmentActivity.KEY_ADDITIONAL_ARGS_FOR_FRAGMENT);
                    reducedParams.putInt("visibleTimes", visibleTimes);
                }
                ...
                Logger.dc(Analysis.TAG, "Page: %s, %s", pageName, reducedParams);
    ...
    public static void sendEvent(String event) {
        sendEventWithExtra(event, "default", "");
    }

    public static void sendEvent(String event, String action) {
        sendEventWithExtra(event, action, "");
    }
