...
public class NotificationUtils {
    ...
    public static void showMessage(String type, long nid, String title, String msg, Intent intent) {
        ...
        Notification notification = new Builder(App.get()).setSmallIcon(R.drawable.ic_push_notification).setTicker(msg).setContentIntent(pendingForwardIntent).setDeleteIntent(PendingIntent.getBroadcast(App.get(), -2, deleteIntent, 1073741824)).setContentText(msg).setContentTitle(title).setLights(Res.getColor(R.color.palette_day_blue), StatConstant.DEFAULT_MAX_EVENT_COUNT, 3000).setAutoCancel(true).setStyle(new BigTextStyle().bigText(msg)).build();
        mNotificationManager.cancel(type, 0);
        mNotificationManager.notify(type, 0, notification);
    }
