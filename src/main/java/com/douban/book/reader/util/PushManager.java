...
public class PushManager {
    ...
    public static void registerDevice(String clientId) {
        CharSequence oldClientId = Pref.ofApp().getString(Key.APP_PUSH_SERVICE_CLIENT_ID, "");
    ...
    private static void asyncRegister(final String clientId) {
        TaskController.run(new Runnable() {
            public void run() {
                try {
                    PushManager.sPushClient.put(RequestParam.json().append("client_id", clientId));
