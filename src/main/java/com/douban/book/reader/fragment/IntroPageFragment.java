...
public class IntroPageFragment extends BaseCardFragment {
    ...
    void refreshViews() {
        if (this.mPageMeta != null) {
            refreshTitle();
            Card introCard = ((TextCard) new SimpleTextCard(App.get()).noDivider()).content(this.mPageMeta.description).firstLineIndent(Indent.ALL);
            ...
            for (final Topic topic : this.mPageMeta.topicList) {
                addCard(((TextCard) ((TextCard) ((TextCard) ((TextCard) new TextCard(App.get()).title(BaseWidgetCard.getTitleWithIcon(R.drawable.v_topic, topic.title))).titleColorArray(R.array.blue)).subTitle(topic.description)).moreBtnVisible(true)).clickListener(new OnClickListener() {
