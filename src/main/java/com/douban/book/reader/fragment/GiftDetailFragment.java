...
public class GiftDetailFragment extends BaseCardFragment {
    ...
    void updateView(final Gift gift) {
        if (gift != null) {
            ...
            if (gift.isMine()) {
                addCard(((ButtonCard) new ButtonCard(getContext()).text(RichText.textWithIcon((int) R.drawable.v_read, (int) R.string.btn_to_my_own_works)).contentPaddingBottomResId(R.dimen.general_subview_vertical_padding_normal)).clickListener(new OnClickListener() {
                    public void onClick(View v) {
                        WorksProfileFragment_.builder().worksId(gift.works.id).build().showAsActivity(GiftDetailFragment.this);
                    }
                }));
                addCard(((ButtonCard) ((ButtonCard) ((ButtonCard) new ButtonCard(getContext()).text(RichText.textWithIcon((int) R.drawable.v_share, (int) R.string.btn_share_my_gifts)).contentPaddingTop(0)).contentPaddingBottomResId(R.dimen.general_subview_vertical_padding_normal)).noDivider()).clickListener(new OnClickListener() {
                    public void onClick(View v) {
                        ShareGiftEditFragment_.builder().uuid(gift.uuid).build().showAsActivity(GiftDetailFragment.this);
                    }
                }));
                addCard(((TextCard) new SimpleTextCard(getContext()).content(Res.getString(R.string.tips_for_received_gifts)).textColorArray(R.array.secondary_text_color).paddingVerticalResId(R.dimen.general_subview_vertical_padding_normal)).noDivider());
            } else {
