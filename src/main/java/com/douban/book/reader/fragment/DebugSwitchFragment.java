...
public class DebugSwitchFragment extends BasePreferenceFragment {
    ...
    private Preference createClearTokenPref(final Preference userInfoPref) {
        Preference preference = new Preference(App.get());
            ...
            public boolean onPreferenceClick(Preference preference) {
                new Builder().setMessage((int) R.string.dialog_message_clear_token_confirm).setPositiveButton((int) R.string.dialog_button_ok, new OnClickListener() {
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        DebugSwitchFragment.this.updateUserInfoPref(userInfoPref);
                    }
    ...
    private Preference createLoginWithRefreshTokenPref(final Preference userInfoPref) {
        Preference preference = new Preference(App.get());
        preference.setTitle(R.string.setting_item_login_with_refresh_token);
        preference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
            public boolean onPreferenceClick(Preference preference) {
                DebugSwitchFragment.this.refreshLogin(userInfoPref);
                return true;
            }
        });
        return preference;
    ...
    private Preference createLogoutPref(final Preference userInfoPref) {
        Preference preference = new Preference(App.get());
    ...
    private Preference createApiConfPref() {
        final ListPreference apiConfPref = new ListPreference(getActivity());
        ...
        TaskController.run(new Runnable() {
            public void run() {
                DebugSwitchFragment.this.fillDataToListPreference(apiConfPref, DebugSwitchFragment.this.getApiConfList());
            }
        });
    ...
    private void fillDataToListPreference(final ListPreference preference, final List<CharSequence> dataList) {
        App.get().runOnUiThread(new Runnable() {
            public void run() {
                try {
                    CharSequence[] dataArray = (CharSequence[]) dataList.toArray(new CharSequence[0]);
                    preference.setEntries(dataArray);
                    preference.setEntryValues(dataArray);
                } catch (Exception e) {
                }
            }
        });
