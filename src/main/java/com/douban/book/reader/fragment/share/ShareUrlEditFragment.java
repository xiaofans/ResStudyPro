...
public class ShareUrlEditFragment extends BaseShareEditFragment {
    ...
    protected String getContentTitle() {
        if (this.mHtmlDocument != null) {
            ...
            while (it.hasNext()) {
                if (StringUtils.isNotEmpty(((Element) it.next()).attr("content"))) {
                    return ((Element) it.next()).attr("content");
                }
            }
    ...
    protected void postToServer(String content) throws DataLoadException {
        try {
            new JsonClient("url/rec").post(((JsonRequestParam) ((JsonRequestParam) RequestParam.json().append("url", this.url)).appendIfNotEmpty("text", content)).appendShareTo(getShareTo()));
        } catch (Throwable e) {
    ...
    private View createBottomView() {
        TextView textView = new TextView(App.get(), null, R.style.AppWidget.Text.Secondary);
