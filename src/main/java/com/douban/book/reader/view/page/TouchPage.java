...
public class TouchPage extends RelativeLayout implements OnClickListener, AnimatorListener {
    ...
    private static final long LONG_PRESSED_PERIOD = ((long) ViewConfiguration.getLongPressTimeout());
    private static final float MAX_DRAGGABLE_SLOP = ((float) (App.get().getPageHeight() / 2));
    ...
    Touchable mActiveTouchable = null;
    private boolean mAllowFlipNext = true;
    private boolean mAllowFlipPrev = true;
    Book mBook;
    int mBookId;
    ...
    float mDraggedY;
    private Runnable mFireFlipNext = new Runnable() {
        public void run() {
    ...
    private Runnable mFireFlipPrevious = new Runnable() {
        public void run() {
            TouchPage.this.mFlipEventPosted = false;
    ...
    private Runnable mFireTextSelect = new Runnable() {
        public void run() {
    ...
    private boolean mFlipEventPosted = false;
    private RectF mFlipNextArea = new RectF();
    private RectF mFlipPrevArea = new RectF();
    BalloonFrame mFootnoteFrame = null;
    ParagraphView mFootnoteText = null;
    public boolean mHasTextSelection = false;
    ...
    float mPinOffsetX;
    float mPinOffsetY;
    ArrayList<Touchable> mPinTouchableArray = new ArrayList();
    AbsoluteLayout mPopupBase = null;
    private SelectionManager mSelectionManager = SelectionManager_.getInstance_(App.get());
    boolean mShouldToggleBookmarkStatus = false;
    private int mShowPageNo;
    boolean mTextSelectable;
    private HashMap<Integer, TouchPage> mTouchPages = new HashMap();
    float mTouchedDownX;
    float mTouchedDownY;
    TextView mTvBookmarkGuide;
    TextView mTvBookmarkPage;
    TouchPage mViewAfterFlip = null;
    ...
    public void setPage(int bookId, int pageNo) {
        ...
        if (chapter != null) {
            ...
            this.mPageView.setLayoutParams(new LayoutParams(-1, -1));
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        Logger.d(Tag.TOUCHEVENT, "TouchPage.onInterceptTouchEvent: " + event, new Object[0]);
        if (isPopupShowing()) {
            boolean handled = this.mPopupBase.dispatchTouchEvent(event);
            Logger.d(Tag.TOUCHEVENT, "TouchPage.onInterceptTouchEvent.dispatchToPopupBase returned " + handled, new Object[0]);
            if (!handled) {
                hidePopups();
                requestDisallowInterceptTouchEvent(true);
            }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        int pointerIndex;
        Logger.d(Tag.TOUCHEVENT, "TouchPage.onTouchEvent: " + event, new Object[0]);
        switch (event.getAction() & 255) {
            case 0:
                this.mTouchedDownX = event.getX();
                this.mTouchedDownY = event.getY();
            ...
            case 2:
                ...
                if (this.mIsBeingDragged) {
                    ...
                    if (this.mTouchedDownY < ((float) TOUCH_SLOP)) {
                        Logger.d(Tag.TOUCHEVENT, String.format("interrupt drag from top area touchDownY %f touch slop %d", new Object[]{Float.valueOf(this.mTouchedDownY), Integer.valueOf(TOUCH_SLOP)}), new Object[0]);
            ...
            case 6:
                ...
                if (MotionEventCompat.getPointerId(event, releasedPointerIndex) == this.mActivePointerId) {
                    int newPointerIndex = releasedPointerIndex == 0 ? 1 : 0;
    ...
    private void showBookmark() {
        ...
        if (this.mTvBookmarkPage != null) {
            ...
            this.mTvBookmarkPage.setTextSize(0, Res.getDimension(R.dimen.font_size_reader_header) * (displayPageNo >= 1000 ? 0.75f : 1.0f));
    ...
    private void showFootnote(FootnoteTouchable footnoteTouchable) {
        ...
        this.mFootnoteText.setTextSize(0, textSize);
            ...
            if (layoutBase != null) {
                ...
                for (Touchable touchable : touchables) {
                    if (touchable instanceof LinkTouchable) {
                        final Uri uri = ((LinkTouchable) touchable).link;
                        if (!urisAlreadyAdded.contains(uri)) {
                            addExtraCommand(layoutBase, UriUtils.toStringRemovingScheme(uri), new OnClickListener() {
                                public void onClick(View v) {
                                    PageOpenHelper.from(TouchPage.this).open(uri);
                                    TouchPage.this.hidePopups();
                                    TouchPage.this.dismissTextSelection();
                                }
                            });
    ...
    public void onClick(View v) {
        Range range = this.mSelectionManager.getSelectionRange();
        switch (v.getId()) {
            case R.id.gallery_popup_menu_correct:
            ...
            case R.id.popup_menu_add_underline:
                try {
            ...
            case R.id.popup_menu_delete_underline:
                try {
            ...
            case R.id.popup_menu_share:
                ...
                break;
            case R.id.popup_menu_copy:
                CharSequence selectedText = StringUtils.truncate(this.mBook.getText(range), 300);
