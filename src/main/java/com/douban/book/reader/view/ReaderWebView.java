...
public class ReaderWebView extends WebView {
    ...
    public void loadUrlWithOAuthRedirect(String url) {
        if (!StringUtils.isEmpty(url)) {
            ...
            try {
                loadUrl(String.format("%s/accounts/auth2_redir?apikey=%s&url=%s", new Object[]{Pref.ofApp().getString(Key.APP_OAUTH_BASE_URL, Constants.OAUTH2_AUTH_URI_BASE), Pref.ofApp().getString(Key.APP_CLIENT_ID, Constants.APP_KEY), URLEncoder.encode(url, "UTF-8")}), headers);
    ...
    private void initCookieIfNeeded() {
        String domain = Pref.ofApp().getString(Key.ARK_HOST, "read.douban.com");
