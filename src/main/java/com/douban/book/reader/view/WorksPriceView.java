...
public class WorksPriceView extends RelativeLayout {
    ...
    public void showPriceFor(final Works works) {
        if (works != null) {
            ...
            if (works.isPromotion()) {
                ...
                setOnClickListener(new OnClickListener() {
                    public void onClick(View v) {
                        WorksListFragment_.builder().uri(WorksListUri.builder().type(Type.PROMOTION).id(works.promotion.event_id).display(Display.PRICE).build()).build().showAsActivity(WorksPriceView.this);
                    }
                });
            }
        }
