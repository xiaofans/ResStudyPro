...
public class TextPageView extends AbsPageView implements TextSelectable {
    ...
    private static final float NOTE_LABEL_HEIGHT = ((float) Utils.dp2pixel(16.0f));
    private static final float NOTE_MARK_HEIGHT = ((float) Utils.dp2pixel(20.0f));
    private static final float NOTE_MARK_MARGIN = ((float) Utils.dp2pixel(2.0f));
    private static final float NOTE_MARK_WIDTH = ((float) Utils.dp2pixel(17.0f));
    ...
    Bitmap mDrawingCacheBitmap = null;
    boolean mDrawingCacheUpdated = false;
    FrontLayerView mFrontLayer;
    private List<Touchable> mNoteMarkTouchableArray = new ArrayList();
    Paint mPaint = new Paint();
    private List<Touchable> mParagraphTouchableArray = new ArrayList();
    int mTextAreaMarginLeft;
    private Collection<Annotation> mUnderlines = null;
        ...
        try {
            synchronized (sDrawingCacheBitmapStack) {
                while (this.mDrawingCacheBitmap == null && !sDrawingCacheBitmapStack.isEmpty()) {
                    this.mDrawingCacheBitmap = (Bitmap) ((SoftReference) sDrawingCacheBitmapStack.pop()).get();
                }
                ...
                if (this.mDrawingCacheBitmap == null) {
                    this.mDrawingCacheBitmap = Bitmap.createBitmap(Dimen.getRealWidth(), Dimen.getRealHeight(), Config.ARGB_8888);
                }
            }
        } catch (OutOfMemoryError e) {
            Logger.e(this.TAG, e);
