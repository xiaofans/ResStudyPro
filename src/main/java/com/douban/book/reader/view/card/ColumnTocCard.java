...
public class ColumnTocCard extends Card<ColumnTocCard> {
    ...
    void updateContent(List<TocItem> tocItemList) {
        ...
        if (tocItemList == null || tocItemList.isEmpty()) {
            ...
            for (final TocItem tocItem : tocItemList.subList(0, Math.min(5, tocItemList.size()))) {
                ...
                tocItemView.setOnClickListener(new OnClickListener() {
        ...
        if (tocItemList == null || tocItemList.size() <= 5) {
            z = true;
        } else {
            z = false;
        }
