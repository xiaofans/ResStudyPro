...
public class SyncedRestClient<T extends Identifiable> extends RestClient<T> {
    private static RealmDataConsumer<PendingRequest> mConsumer = new RealmDataConsumer<PendingRequest>() {
    ...
    private void executePendingRequestsForId(final Object id) throws RestException {
        performRequestList(new RealmDataFilter<PendingRequest>() {
            public RealmResults<PendingRequest> filter(Realm realm) {
                return realm.where(PendingRequest.class).equalTo("resourceType", SyncedRestClient.this.getResourceType()).equalTo("resourceId", SyncedRestClient.this.formatResourceId(id)).findAllSorted("createTime");
            }
        });
    }
