...
public class SessionManager {
    ...
    private final Uri baseUri = Uri.parse(Pref.ofApp().getString(Key.APP_OAUTH_BASE_URL, Constants.OAUTH2_AUTH_URI_BASE)).buildUpon().appendEncodedPath("service/auth2/token").build();
    private final RestClient<Session> mRestClient = new RestClient(this.baseUri, Session.class);
    ...
    public void clearSession() {
        ...
        Pref.ofApp().remove("access_token");
    ...
    public String getAccessToken() {
        ...
        return Pref.ofApp().getString("access_token");
    }
    ...
    public String getRefreshToken() {
        ...
        return Pref.ofApp().getString("refresh_token");
    }
    ...
    public Session retrieveByRefreshToken() throws RestException {
        return (Session) this.mRestClient.post(((FormRequestParam) baseRequestParam().append(WBConstants.AUTH_PARAMS_GRANT_TYPE, "refresh_token")).append("refresh_token", getRefreshToken()));
    }
    ...
    public Session retrieveByDeviceId() throws RestException {
        return (Session) new RestClient("account/register_device", Session.class).post((JsonRequestParam) ((JsonRequestParam) RequestParam.json().append("key", Pref.ofApp().getString(Key.APP_CLIENT_ID, Constants.APP_KEY))).append("secret", Pref.ofApp().getString(Key.APP_CLIENT_SECRET, Constants.APP_SECRET)));
    }

    private FormRequestParam baseRequestParam() {
        return (FormRequestParam) ((FormRequestParam) RequestParam.form().append("client_id", Pref.ofApp().getString(Key.APP_CLIENT_ID, Constants.APP_KEY))).append(WBConstants.AUTH_PARAMS_CLIENT_SECRET, Pref.ofApp().getString(Key.APP_CLIENT_SECRET, Constants.APP_SECRET));
    }
