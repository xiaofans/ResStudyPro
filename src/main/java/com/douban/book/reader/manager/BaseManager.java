...
public class BaseManager<T extends Identifiable> {
    ...
    private void asyncRefreshLocalCache(final Object id) {
        TaskController.run(new Runnable() {
            public void run() {
                try {
                    BaseManager.this.addToCache(id == null ? (Identifiable) BaseManager.this.mRestClient.getEntity() : (Identifiable) BaseManager.this.mRestClient.get(id));
                } catch (Exception e) {
                    Logger.e(BaseManager.this.TAG, e, "Failed to get object %s from server.", id);
                }
            }
        });
    }
