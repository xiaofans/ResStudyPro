...
public class ReviewManager extends BaseManager<Review> {
    ...
    public Lister<Review> listForWorks(int worksId) {
        return defaultLister().filter(new DataFilter().append("works_id", Integer.valueOf(worksId)));
    }
    ...
    public Review addReview(int worksId, String comment, int rating, RequestParam<?> shareParam) throws DataLoadException {
        ...
        shareParam.append("works_id", Integer.valueOf(worksId)).append("comment", comment).append(ReviewEditFragment_.RATING_ARG, Integer.valueOf(rating));
    ...
    public void newComment(int reviewId, String content) throws DataLoadException {
        getCommentManager(reviewId).post(RequestParam.json().append("content", content));
    ...
    public List<Review> getHottestReviewsForWorks(int worksId, int limit) throws DataLoadException {
        return defaultLister().filter(new DataFilter().append("works_id", Integer.valueOf(worksId)).append("order", WBConstants.GAME_PARAMS_SCORE).append("limit", Integer.valueOf(limit))).loadMore();
    }

    public Review getByLegacyId(int reviewId) throws DataLoadException {
        return (Review) getSubManager("mapping", Review.class).get(RequestParam.queryString().append("id", Integer.valueOf(reviewId)));
    }
