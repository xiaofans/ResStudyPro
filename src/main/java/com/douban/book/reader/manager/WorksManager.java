...
public class WorksManager extends BaseManager<Works> {
    ...
    public WorksManager() {
        ...
        excludes("toc");
        cache(new PinnableCache(new MemoryCache(), new BaseFileCache<Works>(Works.class) {
            protected File getFile(Object id) {
                return FilePath.worksMeta(Integer.parseInt(String.valueOf(id)));
            }
        }, Works.class));
    }
    ...
    public void pinIfAbsent(int worksId) {
        try {
            Works works = (Works) getFromCache(Integer.valueOf(worksId));
        } catch (Throwable e1) {
            Logger.ec(this.TAG, e1);
        }
    }
    ...
    public Manifest getManifest(int worksId) throws DataLoadException {
        try {
            return (Manifest) getSubManagerForId(Integer.valueOf(worksId), "manifest", Manifest.class).post((JsonRequestParam) ((JsonRequestParam) RequestParam.json().append("pk", ArkKeyPair.getPublic())).append("version", Integer.valueOf(3)));
        } catch (Throwable e) {
    ...
    public Works getWorksByLegacyColumnId(int columnId) throws DataLoadException {
        try {
            Works works = (Works) new RestClient("column/mapping", Works.class).get(RequestParam.queryString().append("id", Integer.valueOf(columnId)));
            addToCache(works);
            return works;
        } catch (RestException e) {
            throw wrapDataLoadException(e);
    ...
    public void correctRange(int worksId, Range range, String comment) throws DataLoadException {
        getSubManagerForId(Integer.valueOf(worksId), "erratum").post(((JsonRequestParam) RequestParam.json().appendRange(range)).appendIfNotEmpty("note", comment));
    }
