...
public class AnnotationManager extends BaseSyncedManager<Annotation> {
    ...
    public void updateNote(UUID uuid, String note) throws DataLoadException {
        ...
        try {
            update(uuid, (JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) RequestParam.json().append("note", note)).append(Column.NOTE_UPDATE_TIME, updateTime)).append("update_time", updateTime));
    ...
    public Annotation setIsPrivate(UUID uuid, boolean isPrivate) throws DataLoadException {
        return (Annotation) onlineUpdate(uuid, (JsonRequestParam) ((JsonRequestParam) RequestParam.json().append(Column.PRIVACY, isPrivate ? Privacy.PRIVATE : Privacy.PUBLIC)).append("update_time", new Date()));
    }
    ...
    public void refresh() throws DataLoadException {
        ...
        Pref.ofWorks(this.mWorksId).set(Key.WORKS_ANNOTATION_LAST_UPDATED_AT, refresh(new DataFilter().append("works_id", Integer.valueOf(this.mWorksId)).appendIfNotNull("since", lastUpdatedAt)));
    ...
    private List<Annotation> loadFromNetwork() throws DataLoadException {
        Lister<Annotation> lister = list().cache(null).filter(new DataFilter().append("works_id", Integer.valueOf(this.mWorksId)));
    ...
    private List<Annotation> loadFromLocal(String type) throws DataLoadException {
        try {
            QueryBuilder<Annotation, Object> queryBuilder = OrmUtils.getDao(Annotation.class).queryBuilder().orderBy(Column.END_POSITION, true).orderBy("create_time", true);
    ...
    public Annotation getPreviousNote(Annotation annotation) {
        ...
        try {
            ...
            where = where.and(where, where.and(where.or(where.lt(Column.END_POSITION, Long.valueOf(annotation.endPosition)), where.eq(Column.END_POSITION, Long.valueOf(annotation.endPosition)).and().lt("create_time", annotation.createTime), new Where[0]), where.ne("uuid", annotation.uuid), new Where[0]), new Where[0]);
            QueryBuilder<Annotation, Object> queryBuilder = dao.queryBuilder();
            queryBuilder.orderBy(Column.END_POSITION, false).orderBy("create_time", false).setWhere(where);
            return (Annotation) queryBuilder.queryForFirst();
        } catch (SQLException e) {
            Logger.e(this.TAG, e);
            return null;
    ...
    public Annotation getNextNote(Annotation annotation) {
        ...
        try {
            Dao<Annotation, Object> dao = OrmUtils.getDao(Annotation.class);
            Where<Annotation, Object> where = whereValidForCurrentUser(Type.NOTE);
            where = where.and(where, where.and(where.or(where.gt(Column.END_POSITION, Long.valueOf(annotation.endPosition)), where.eq(Column.END_POSITION, Long.valueOf(annotation.endPosition)).and().gt("create_time", annotation.createTime), new Where[0]), where.ne("uuid", annotation.uuid), new Where[0]), new Where[0]);
            QueryBuilder<Annotation, Object> queryBuilder = dao.queryBuilder();
            queryBuilder.orderBy(Column.END_POSITION, true).orderBy("create_time", true).setWhere(where);
            return (Annotation) queryBuilder.queryForFirst();
        } catch (SQLException e) {
            Logger.e(this.TAG, e);
            return null;
    ...
    private List<Annotation> listUnIndexed() throws DataLoadException {
        try {
            ...
            Where eq = where.eq("works_id", Integer.valueOf(this.mWorksId));
            ...
            whereArr[0] = where.raw(String.format("%s = %d", new Object[]{"is_deleted", Integer.valueOf(0)}), new ArgumentHolder[0]);
    ...
    private Where<Annotation, Object> whereValid(String type) throws SQLException {
        Where<Annotation, Object> where = OrmUtils.getDao(Annotation.class).queryBuilder().where();
        where.eq("works_id", Integer.valueOf(this.mWorksId)).and().gt(Column.START_POSITION, Integer.valueOf(0)).and().gt(Column.END_POSITION, Integer.valueOf(0)).and().raw(String.format("%s = %d", new Object[]{"is_deleted", Integer.valueOf(0)}), new ArgumentHolder[0]);
