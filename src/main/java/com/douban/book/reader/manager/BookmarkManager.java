...
public class BookmarkManager extends BaseLegacySyncedManager<Bookmark> {
    private static LruCache<Integer, BookmarkManager> sInstances = new LruCache(5);
    ...
    public void refresh() throws DataLoadException {
        ...
        Pref.ofWorks(this.mWorksId).set(Key.WORKS_BOOKMARK_LAST_UPDATED_AT, refresh(new DataFilter().append("works_id", Integer.valueOf(this.mWorksId)).appendIfNotNull("since", lastUpdatedAt)));
    ...
    private List<Bookmark> loadFromNetwork() throws DataLoadException {
        Lister<Bookmark> lister = list().cache(null).filter(new DataFilter().append("works_id", Integer.valueOf(this.mWorksId)));
    ...
    private List<Bookmark> loadFromLocal() throws DataLoadException {
        try {
            return OrmUtils.getDao(Bookmark.class).queryBuilder().orderBy(Column.SORT_INDEX, true).where().eq("works_id", Integer.valueOf(this.mWorksId)).and().gt(Column.SORT_INDEX, Integer.valueOf(0)).and().raw(String.format("%s = %d", new Object[]{"is_deleted", Integer.valueOf(0)}), new ArgumentHolder[0]).query();
        } catch (Throwable e) {
    ...
    private List<Bookmark> listUnIndexed() throws DataLoadException {
        try {
            return OrmUtils.getDao(Bookmark.class).queryBuilder().where().eq("works_id", Integer.valueOf(this.mWorksId)).and().le(Column.SORT_INDEX, Integer.valueOf(0)).and().raw(String.format("%s = %d", new Object[]{"is_deleted", Integer.valueOf(0)}), new ArgumentHolder[0]).query();
        } catch (Throwable e) {
            throw new DataLoadException(e);
        }
    }
    ...
    private Where<Bookmark, Object> whereInRange(Range range) throws SQLException {
        return OrmUtils.getDao(Bookmark.class).queryBuilder().where().eq("works_id", Integer.valueOf(this.mWorksId)).and().between(Column.SORT_INDEX, Long.valueOf(range.startPosition.getActualPosition()), Long.valueOf(range.endPosition.getActualPosition())).and().raw(String.format("%s = %d", new Object[]{"is_deleted", Integer.valueOf(0)}), new ArgumentHolder[0]);
    }
}
