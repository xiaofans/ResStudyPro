...
public class GiftPackManager extends BaseManager<GiftPack> {
    ...
    public GiftPack create(int worksId, int eventId, int quantity, CharSequence message) throws DataLoadException {
        return (GiftPack) post(((JsonRequestParam) ((JsonRequestParam) ((JsonRequestParam) RequestParam.json().append("works_id", Integer.valueOf(worksId))).appendIf(eventId > 0, "event_id", Integer.valueOf(eventId))).append("quantity", Integer.valueOf(quantity))).append("message", message));
    }

    public void updateGiftNote(int giftPackId, CharSequence newGiftNote) throws DataLoadException {
        update(Integer.valueOf(giftPackId), RequestParam.json().append("message", newGiftNote));
