...
public class Toc {
    ...
    private static final String TAG = Toc.class.getSimpleName();
    ...
    public String getTitleForPage(int page) {
        TocItem tocItem = getTocItemForPosition(Book.get(this.mWorksId).getPageInfo(page).getRange().endPosition, Integer.MAX_VALUE);
        if (tocItem != null) {
    ...
    public TocItem getTocItemForReadingProgress() {
        return getTocItemForPosition(ProgressManager.ofWorks(this.mWorksId).getLocalProgress().getPosition(), Integer.MAX_VALUE);
    }
