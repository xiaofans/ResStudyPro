...
public class Format {
    ...
    public float textRatio = TEXTSIZE_RATIO_NORMAL;
    ...
    public Format applyStyle(String styleName) {
        ...
        if (StringUtils.inList(styleName, "title", STYLE_SUBTITLE, STYLE_AUTHOR, STYLE_TRANSLATOR)) {
            builder.indent(false);
            builder.firstLineIndent(false);
        }
        if (StringUtils.inList(styleName, "title", STYLE_SUBTITLE)) {
            builder.bold(true);
        }
        if (StringUtils.inList(styleName, "title")) {
