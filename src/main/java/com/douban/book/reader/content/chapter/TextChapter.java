...
public class TextChapter extends ContentChapter {
    private final WatchDogTimer mWatchDogTimer = new WatchDogTimer(new Callback() {
    ...
    public CharSequence getText(Range range) {
        ...
        if (end.packageId == this.mPackageId && end.isValid()) {
            ...
            endOffset = end.paragraphOffset < Integer.MAX_VALUE ? end.paragraphOffset + 1 : end.paragraphOffset;
        } else {
        ...
        while (i <= end.paragraphIndex) {
            ...
            if (paragraph != null && paragraph.canPinStop()) {
                sb.append(paragraph.getPrintableText(i == startPara ? startOffset : 0, i == endPara ? endOffset : Integer.MAX_VALUE));
                if (i < endPara) {
                    sb.append("\n");
                }
            }
