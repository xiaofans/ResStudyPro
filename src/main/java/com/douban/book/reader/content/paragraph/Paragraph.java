...
public abstract class Paragraph {
    ...
    private static final float SELECTION_PIN_LINE_WIDTH = ((float) Utils.dp2pixel(3.0f));
    ...
    private float getTextAreaLeftMargin() {
        ...
        if (this.mDecorator != null) {
            result = 0.0f + this.mDecorator.getInsetLeft();
        }
        if (this.mIndentFlag) {
    ...
    public CharSequence getPrintableText(int charOffset) {
        return getPrintableText(charOffset, Integer.MAX_VALUE);
    }
    ...
    public void setBlockQuote(boolean blockQuote, @ArrayRes @ColorRes int quoteLineColorRes) {
        if (blockQuote) {
