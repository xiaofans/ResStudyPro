...
public class LabelSpan extends ReplacementSpan {
    ...
    private static final float LABEL_VERTICAL_PADDING = ((float) Utils.dp2pixel(1.0f));
    ...
    public int getSize(Paint paint, CharSequence text, int start, int end, FontMetricsInt fm) {
        this.mWidth = Math.round((paint.measureText(text, start, end) * 0.8f) + ((paint.getTextSize() * PADDING_RATIO) * 2.0f));
        return this.mWidth;
    }
    ...
    public void draw(Canvas canvas, CharSequence text, int start, int end, float x, int top, int y, int bottom, Paint paint) {
        ...
        Paint innerPaint = PaintUtils.obtainPaint(paint.getTextSize() * 0.8f);
