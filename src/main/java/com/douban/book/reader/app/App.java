...
public class App extends Application {
    ...
    public static ContextThemeWrapper getThemed() {
        return new ContextThemeWrapper(get(), com.douban.book.reader.theme.Theme.isNight() ? R.style.AppBaseTheme : R.style.AppBaseTheme.Light);
    }
    ...
    private void asyncInit() {
        ...
        try {
            for (ShelfItem shelfItem : ShelfManager_.getInstance_(this).getAll()) {
                WorksManager_.getInstance_(this).pinIfAbsent(shelfItem.id);
            }
        } catch (Throwable e2) {
            Logger.ec(TAG, e2);
        }
