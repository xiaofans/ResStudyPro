        ...
        switch (value.type) {
            ...
            case 3:
                ...
                if (servingValue.mapKey.length != servingValue.mapValue.length) {
                    logAndThrow("Uneven map keys (" + servingValue.mapKey.length + ") and map values (" + servingValue.mapValue.length + ")");
                }
                ...
                while (i$ < len$) {
                    index2 = index + 1;
                    toAdd.mapKey[index] = expandValue(arr$[i$], resource, expandedValues, pendingExpansions);
                    i$++;
                    index = index2;
                }
