...
class AppInvokeCmdExecutor implements CmdExecutor<AppInvokeCmd> {
    ...
    private static void showNotification(Context ctx, AppInvokeCmd cmd) {
        SDKNotificationBuilder.buildUpon().setNotificationContent(cmd.getNotificationText()).setNotificationPendingIntent(buildInvokePendingIntent(ctx, cmd)).setNotificationTitle(cmd.getNotificationTitle()).setTickerText(cmd.getNotificationText()).build(ctx).show(2);
    }
    ...
    private static PendingIntent buildInvokePendingIntent(Context ctx, AppInvokeCmd cmd) {
        ...
        if (intent != null) {
            List<ResolveInfo> activities = ctx.getPackageManager().queryIntentActivities(intent, 65536);
