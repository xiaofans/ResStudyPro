...
public class DownloadInfo implements Serializable, Cloneable {
    ...
    public void refreshSpeed() {
        ...
        if (this.mCurrentBytes != this.mLastBytes) {
            if (this.a != 0) {
                this.mDownSpeed = ((long) (Math.max(0, this.mCurrentBytes - this.mLastBytes) * 1000)) / currentTimeMillis;
            } else {
                this.mDownSpeed = 0;
            }
    ...
    public long restartTime() {
        return this.mRetryAfter > 0 ? this.mLastMod + ((long) this.mRetryAfter) : this.mLastMod + ((long) (((this.mFuzz + 1000) * 30) * (1 << (this.mNumFailed - 1))));
    }
}
