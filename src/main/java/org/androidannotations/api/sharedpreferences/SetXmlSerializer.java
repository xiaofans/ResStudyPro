...
public final class SetXmlSerializer {
    ...
    public static String serialize(Set<String> set) {
        ...
        try {
            ...
            serializer.startTag("", SET_TAG);
            for (String string : set) {
                serializer.startTag("", STRING_TAG).text(string).endTag("", STRING_TAG);
            }
            serializer.endTag("", SET_TAG).endDocument();
    ...
    public static Set<String> deserialize(String data) {
        ...
        try {
            ...
            parser.next();
            parser.require(2, "", SET_TAG);
            while (parser.next() != 3) {
                parser.require(2, "", STRING_TAG);
                parser.next();
