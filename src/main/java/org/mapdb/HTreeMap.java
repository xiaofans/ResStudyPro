...
public class HTreeMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, MapWithModificationListener<K, V> {
    ...
    protected static final Serializer<long[][]> DIR_SERIALIZER = new Serializer<long[][]>() {
        static final /* synthetic */ boolean $assertionsDisabled = (!HTreeMap.class.desiredAssertionStatus());
    ...
    class Entry2 implements Entry<K, V> {
        ...
        public boolean equals(Object o) {
            return (o instanceof Entry) && HTreeMap.this.hasher.equals(this.key, ((Entry) o).getKey());
        }
    ...
    abstract class HashIterator {
        ...
        private LinkedNode[] advance(int lastHash) {
            ...
            try {
                ...
                while (true) {
                    ...
                    int pos = (lastHash >>> (level * 7)) & 127;
                    if (dir[pos >>> 3] != null && dir[pos >>> 3][pos & 7] != 0 && (dir[pos >>> 3][pos & 7] & 1) != 1) {
                        dirRecid = dir[pos >>> 3][pos & 7] >>> 1;
                        level--;
                    } else if (level == 0) {
                        lastHash = ((lastHash >>> (level * 7)) + 1) << (level * 7);
                    } else {
                        lastHash++;
                    }
                }
        ...
        private LinkedNode[] findNextLinkedNode(int hash) {
            ...
            while (segment < 16) {
                ...
                try {
                    ...
                    LinkedNode[] ret = findNextLinkedNodeRecur(HTreeMap.this.segmentRecids[segment], hash, 3);
                    if (ret != null) {
                        LinkedNode[] arr$ = ret;
                        len$ = arr$.length;
                        i$ = 0;
        ...
        private LinkedNode[] findNextLinkedNodeRecur(long dirRecid, int newHash, int level) {
            ...
            boolean first = true;
            for (int pos = (newHash >>> (level * 7)) & 127; pos < 128; pos++) {
                if (dir[pos >>> 3] != null) {
                    long recid = dir[pos >>> 3][pos & 7];
                    if (recid == 0) {
                        continue;
                    } else if ((1 & recid) == 1) {
                        recid >>= 1;
...
*/
    ...
    public HTreeMap(Engine engine, long counterRecid, int hashSalt, long[] segmentRecids, Serializer<K> keySerializer, Serializer<V> valueSerializer, long expireTimeStart, long expire, long expireAccess, long expireMaxSize, long expireStoreSize, long[] expireHeads, long[] expireTails, Function1<V, K> valueCreator, Hasher hasher, boolean disableLocks) {
        ...
        if (counterRecid < 0) {
            ...
            if (expireMaxSize == 0 || counterRecid != 0) {
                boolean z = (expire == 0 && expireAccess == 0 && expireMaxSize == 0 && expireStoreSize == 0) ? false : true;
                this.expireFlag = z;
                this.expire = expire;
                this.expireTimeStart = expireTimeStart;
                z = (expireAccess == 0 && expireMaxSize == 0 && expireStoreSize == 0) ? false : true;
    ...
    protected LinkedNode<K, V> getInner(Object o, int h, int segment) {
        ...
        while (level >= 0) {
            ...
            int slot = (h >>> (level * 7)) & 127;
            if (!$assertionsDisabled && slot >= 128) {
                throw new AssertionError();
            } else if (dir[slot >>> 3] == null) {
                return null;
            } else {
    ...
    protected V removeInternal(Object key, int segment, int h, boolean removeExpire) {
        ...
        int level = 3;
        dirRecids[3] = this.segmentRecids[segment];
        if ($assertionsDisabled || segment == (h >>> 28)) {
            long[][] jArr;
            int slot;
            long recid;
            while (true) {
                jArr = (long[][]) this.engine.get(dirRecids[level], DIR_SERIALIZER);
                slot = (h >>> (level * 7)) & 127;
                if ($assertionsDisabled || slot <= 127) {
                    if (jArr == null) {
                        jArr = new long[16][];
                    }
                    if (jArr[slot >>> 3] == null) {
                        jArr = (long[][]) Arrays.copyOf(jArr, 16);
                        jArr[slot >>> 3] = new long[8];
                    }
                    recid = jArr[slot >>> 3][slot & 7];
                    if (recid != 0) {
                        if ((1 & recid) != 0) {
                            break;
                        }
            ...
            recid >>>= 1;
            LinkedNode<K, V> ln = (LinkedNode) this.engine.get(recid, this.LN_SERIALIZER);
            long prevRecid = 0;
            LinkedNode<K, V> prevLn = null;
            while (ln != null) {
                LinkedNode<K, V> prevLn2;
                if (this.hasher.equals(ln.key, key)) {
                    if (prevLn != null) {
                        prevLn2 = new LinkedNode(ln.next, prevLn.expireLinkNodeRecid, prevLn.key, prevLn.value);
                        this.engine.update(prevRecid, prevLn2, this.LN_SERIALIZER);
                    } else if (ln.next == 0) {
                        recursiveDirDelete(h, level, dirRecids, jArr, slot);
                        prevLn2 = prevLn;
                    } else {
                        jArr = (long[][]) Arrays.copyOf(jArr, 16);
                        jArr[slot >>> 3] = Arrays.copyOf(jArr[slot >>> 3], 8);
                        jArr[slot >>> 3][slot & 7] = (ln.next << 1) | 1;
                        this.engine.update(dirRecids[level], jArr, DIR_SERIALIZER);
                        prevLn2 = prevLn;
                    }
                    if ($assertionsDisabled || hash(ln.key) == h) {
    ...
    private void recursiveDirDelete(int h, int level, long[] dirRecids, long[][] dir, int slot) {
        ...
        dir2[slot >>> 3] = Arrays.copyOf(dir2[slot >>> 3], 8);
        dir2[slot >>> 3][slot & 7] = 0;
        boolean allZero = true;
        for (long l : dir2[slot >>> 3]) {
            if (l != 0) {
                allZero = false;
                break;
            }
        ...
        if (!allZero) {
            ...
            recursiveDirDelete(h, level + 1, dirRecids, (long[][]) this.engine.get(dirRecids[level + 1], DIR_SERIALIZER), (h >>> ((level + 1) * 7)) & 127);
        }
    }
        ...
        try {
            ...
            while (recid != 0) {
                ...
                if (!$assertionsDisabled && n == ExpireLinkNode.EMPTY) {
                    ...
                    counter++;
                    boolean remove = counter < removePerSegment || (!(this.expire == 0 && this.expireAccess == 0) && n.time + this.expireTimeStart < System.currentTimeMillis());
                    if (!remove) {
                        break;
                    }
                    this.engine.delete(recid, ExpireLinkNode.SERIALIZER);
                    removeInternal(((LinkedNode) this.engine.get(n.keyRecid, this.LN_SERIALIZER)).key, seg, n.hash, false);
                    last = n;
                    recid = n.next;
                } else {
                    throw new AssertionError();
    ...
    public Map<K, V> snapshot() {
        return new HTreeMap(TxEngine.createSnapshotFor(this.engine), this.counter == null ? 0 : this.counter.recid, this.hashSalt, this.segmentRecids, this.keySerializer, this.valueSerializer, 0, 0, 0, 0, 0, null, null, null, null, false);
    }
