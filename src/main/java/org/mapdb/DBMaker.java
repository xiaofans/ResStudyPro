...
public class DBMaker<DBMakerT extends DBMaker<DBMakerT>> {
    ...
    public static <K, V> HTreeMap<K, V> newCacheDirect(double size) {
        return newMemoryDirectDB().transactionDisable().make().createHashMap("cache").expireStoreSize(size).counterEnable().make();
    }

    public static <K, V> HTreeMap<K, V> newCache(double size) {
        return newMemoryDB().transactionDisable().make().createHashMap("cache").expireStoreSize(size).counterEnable().make();
    }
    ...
    public DBMakerT cacheSize(int cacheSize) {
        this.props.setProperty(Keys.cacheSize, "" + cacheSize);
        return getThis();
    }
    ...
    public DBMakerT asyncWriteFlushDelay(int delay) {
        this.props.setProperty(Keys.asyncWriteFlushDelay, "" + delay);
        return getThis();
    }

    public DBMakerT asyncWriteQueueSize(int queueSize) {
        this.props.setProperty(Keys.asyncWriteQueueSize, "" + queueSize);
        return getThis();
    }
    ...
    public DBMakerT freeSpaceReclaimQ(int q) {
        ...
        this.props.setProperty(Keys.freeSpaceReclaimQ, "" + q);
        return getThis();
    }
    ...
    public DBMakerT sizeLimit(double maxSize) {
        this.props.setProperty(Keys.sizeLimit, "" + ((long) (((maxSize * 1024.0d) * 1024.0d) * 1024.0d)));
        return getThis();
    }
    ...
    public Engine makeEngine() {
        ...
        if (readOnly && file == null) {
            ...
            String cache = this.props.getProperty("cache", "hashTable");
    ...
    protected Engine extendCacheLRU(Engine engine) {
        return new LRU(engine, propsGetInt(Keys.cacheSize, 32768), false);
    }
    ...
    protected Engine extendCacheHardRef(Engine engine) {
        return new HardRef(engine, propsGetInt(Keys.cacheSize, 32768), false);
    }

    protected Engine extendCacheHashTable(Engine engine) {
        return new HashTable(engine, propsGetInt(Keys.cacheSize, 32768), false);
    }
