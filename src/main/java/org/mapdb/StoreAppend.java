...
class StoreAppend extends Store {
    static final /* synthetic */ boolean $assertionsDisabled = (!StoreAppend.class.desiredAssertionStatus());
    ...
    public StoreAppend(File file, boolean useRandomAccessFile, boolean readOnly, boolean transactionDisabled, boolean deleteFilesAfterClose, boolean syncOnCommitDisabled, boolean checksum, boolean compress, byte[] password, boolean disableLocks) {
        ...
        this.index = new MemoryVol(false, 0, 20);
        this.file = file;
        ...
        if (parent.exists() && parent.isDirectory()) {
            ...
            if (sortedFiles.isEmpty()) {
                Volume zero = Volume.volumeForFile(getFileFromNum(0), useRandomAccessFile, readOnly, 0, 20, 0);
                zero.ensureAvailable(64);
                zero.putLong(0, HEADER);
                pos = 8;
                for (recid = 1; recid <= 7; recid++) {
                    pos += (long) zero.putPackedLong(pos, 3 + recid);
                    pos += (long) zero.putPackedLong(pos, 2);
                }
                this.maxRecid = 7;
            ...
            for (Tuple2<Long, File> t : sortedFiles) {
                Long num = t.a;
                f2 = (File) t.b;
                Volume vol = Volume.volumeForFile(f2, useRandomAccessFile, readOnly, 0, 20, 0);
                ...
                while (pos <= FILE_MASK) {
                    ...
                    if (recid == END) {
                        ...
                        long indexVal = (num.longValue() << 24) | pos;
    ...
    protected void rollover() {
        if (this.currVolume.getLong(0) != HEADER) {
            ...
            this.currFileNum++;
            this.currVolume = Volume.volumeForFile(getFileFromNum(this.currFileNum), this.useRandomAccessFile, this.readOnly, 0, 20, 0);
    ...
    public <A> long put(A value, Serializer<A> serializer) {
        if ($assertionsDisabled || value != null) {
            ...
            try {
                ...
                long recid = this.maxRecid + 1;
                this.maxRecid = recid;
                this.currPos += (long) this.currVolume.putPackedLong(this.currPos, 3 + recid);
                long indexVal = (this.currFileNum << 24) | this.currPos;
                this.currPos += (long) this.currVolume.putPackedLong(this.currPos, ((long) out.pos) + 2);
    ...
    protected <A> A getNoLock(long recid, Serializer<A> serializer) throws IOException {
        ...
        Volume vol = (Volume) this.volumes.get(indexVal >>> 24);
    ...
    public <A> void delete(long recid, Serializer<A> serializer) {
        if ($assertionsDisabled || recid > 0) {
            ...
            try {
                ...
                this.currPos += (long) this.currVolume.putPackedLong(this.currPos, 2 + recid);
                setIndexVal(recid, (this.currFileNum << 24) | this.currPos);
        ...
        try {
            if (this.indexInTx.isEmpty()) {
                ...
                for (long recid = 0; recid <= this.maxRecid; recid++) {
                    ...
                    if (indexVal != 0) {
                        ff.put(indexVal >>> 24, Boolean.valueOf(true));
                    }
                }
