...
public class StoreDirect extends Store {
    static final /* synthetic */ boolean $assertionsDisabled = (!StoreDirect.class.desiredAssertionStatus());
    ...
    protected void checkHeaders() {
        ...
        } else if (this.index.getUnsignedShort(4) > AbstractSpiCall.DEFAULT_TIMEOUT || this.phys.getUnsignedShort(4) > AbstractSpiCall.DEFAULT_TIMEOUT) {
    ...
    protected long indexHeaderChecksum() {
        long ret = 0;
        for (long offset = 0; offset < 32896; offset += 8) {
            if (offset != 32) {
    ...
    public <A> long put(A value, Serializer<A> serializer) {
        ...
        if ($assertionsDisabled || value != null) {
            ...
            try {
                this.structuralLock.lock();
                long ioRecid = freeIoRecidTake(true);
                long[] indexVals = physAllocate(out.pos, true, false);
                this.structuralLock.unlock();
                lock = this.locks[Store.lockPos(ioRecid)].writeLock();
                lock.lock();
    ...
    protected void put2(DataOutput2 out, long ioRecid, long[] indexVals) {
        if ($assertionsDisabled || this.locks[Store.lockPos(ioRecid)].writeLock().isHeldByCurrentThread()) {
            this.index.putLong(ioRecid, indexVals[0] | 2);
    ...
    protected void update2(DataOutput2 out, long ioRecid) {
        long indexVal = this.index.getLong(ioRecid);
        int size = (int) (indexVal >>> 48);
        boolean linked = (8 & indexVal) != 0;
        if ($assertionsDisabled || this.locks[Store.lockPos(ioRecid)].writeLock().isHeldByCurrentThread()) {
            if (linked || out.pos <= 0 || size <= 0 || size2ListIoRecid((long) size) != size2ListIoRecid((long) out.pos)) {
                ...
                try {
                    ...
                    linkedRecordsIndexVals = physAllocate(out.pos, true, false);
                ...
                long offset = indexVal & MASK_OFFSET;
                this.index.putLong(ioRecid, ((((long) out.pos) << 48) | offset) | 2);
    ...
    public void compact() {
        ...
        try {
            ...
            boolean asyncWriteEnabled = (this.index instanceof ByteBufferVol) && ((ByteBufferVol) this.index).asyncWriteEnabled;
            StoreDirect store2 = new StoreDirect(Volume.fileFactory(compactedFile, rafMode, false, this.sizeLimit, 20, 0, new File(compactedFile.getPath() + DATA_FILE_EXT), new File(compactedFile.getPath() + StoreWAL.TRANS_LOG_FILE_EXT), asyncWriteEnabled), false, false, 5, false, 0, this.checksum, this.compress, this.password, false, 0);
            compactPreUnderLock();
            this.index.putLong(16, this.physSize);
            this.index.putLong(8, this.indexSize);
            this.index.putLong(24, this.freeSize);
            store2.lockAllWrite();
            long recid = longStackTake(120, false);
            while (recid != 0) {
                store2.longStackPut(120, recid, false);
                recid = longStackTake(120, false);
            }
            store2.index.putLong(8, this.indexSize);
            for (long ioRecid = 32896; ioRecid < this.indexSize; ioRecid += 8) {
                byte[] bb = (byte[]) get2(ioRecid, Serializer.BYTE_ARRAY_NOSIZE);
                store2.index.ensureAvailable(8 + ioRecid);
                if (bb == null || bb.length == 0) {
                    store2.index.putLong(ioRecid, 0);
                } else {
                    DataOutput2 out = serialize(bb, Serializer.BYTE_ARRAY_NOSIZE);
                    store2.put2(out, ioRecid, store2.physAllocate(out.pos, true, false));
                }
            }
            File indexFile2 = store2.index.getFile();
            File physFile2 = store2.phys.getFile();
            store2.unlockAllWrite();
            boolean useDirectBuffer = (this.index instanceof MemoryVol) && ((MemoryVol) this.index).useDirectBuffer;
            this.index.sync();
            this.index.close();
            this.index = null;
            this.phys.sync();
            ...
            if (indexFile != null) {
                ...
                if (!indexFile.renameTo(indexFile_)) {
                    ...
                    Factory fac2 = Volume.fileFactory(indexFile, rafMode, false, this.sizeLimit, 20, 0, new File(indexFile.getPath() + DATA_FILE_EXT), new File(indexFile.getPath() + StoreWAL.TRANS_LOG_FILE_EXT), asyncWriteEnabled);
        ...
        if (!$assertionsDisabled && !this.structuralLock.isHeldByCurrentThread()) {
            ...
            if (pos == 8) {
                ...
                if (this.maxUsedIoList == ioList) {
                    while (true) {
                        if (this.index.getLong(this.maxUsedIoList) != 0 || this.maxUsedIoList <= 120) {
                            break;
                        }
                        this.maxUsedIoList -= 8;
                    }
            ...
            this.index.putLong(ioList, ((pos - 6) << 48) | dataOffset);
            return ret;
        } else {
    ...
    protected void longStackPut(long ioList, long offset, boolean recursive) {
        if (!$assertionsDisabled && !this.structuralLock.isHeldByCurrentThread()) {
            ...
            if (dataOffset == 0) {
                ...
                this.phys.putSixLong(8 + listPhysid, offset);
    ...
    protected long freeIoRecidTake(boolean ensureAvail) {
        if (this.spaceReclaimTrack) {
            long longStackTake = longStackTake(120, false);
            if (longStackTake != 0) {
    ...
    public void updateRaw(long recid, ByteBuffer data) {
        long ioRecid = (recid * 8) + 32896;
        if (ioRecid >= this.indexSize) {
            this.indexSize = ioRecid + 8;
            this.index.ensureAvailable(this.indexSize);
        }
        byte[] b = null;
        if (data != null) {
    ...
    public String calculateStatistics() {
        ...
        for (int size = 16; size < 65545; size *= 2) {
            long sum = 0;
